...
- Abstract opening: adopted the clearer Chapter 2 progression from the benefits of minimally invasive surgery, through the challenge of indirect restricted vision, to the motivation for computer-aided intervention before introducing recent surgical-AI advances. The prose is thesis-specific rather than copied from Chapter 2, and the abstract contains no citations.
...
- Chapter 2 dataset-table rule repair: removed overlapping vertical rules introduced by bordered one-column spans and eliminated adjacent normal/thick horizontal separators. A fresh rendered-page inspection confirms single column boundaries and single dataset separators throughout the multipage table; the build remains free of table-specific overflow, undefined citation/reference, duplicate-key, and LaTeX/Biber error diagnostics.
