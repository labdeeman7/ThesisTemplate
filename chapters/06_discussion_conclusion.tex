...
Constructing CholecInstanceSeg required the generation of new instance-level labels across heterogeneous public laparoscopic datasets. Source annotations were converted into a common representation, individual instrument instances were delineated and refined, and the resulting labels were reconciled and subjected to systematic quality control and technical validation. The validation experiments in Chapter~\ref{chap:cholecinstanceseg} demonstrate that contemporary instance-segmentation models can learn from these annotations, while the accompanying analyses document the dataset composition and its limitations. CholecInstanceSeg therefore contributes both an original large-scale annotation resource and evidence that instrument instances can be modelled in routine laparoscopic scenes.
...
CholecTriplet-Seg establishes spatially grounded triplet segmentation as a benchmark problem and provides the instance-aligned annotations and evaluation protocol required to study it. It advances the triplet task from asking what occurs in a frame towards identifying which instrument performs each action and upon which anatomical target.
...
The long-tail and failure analyses show declining performance for less frequent triplets and continuing confusion among anatomically adjacent or visually similar targets. Target prediction consequently continues to constrain complete-triplet performance. TargetFusionNet demonstrates the value of explicit target-aware modelling while identifying the continuing need for better target supervision, contact localisation, and contextual modelling.
...
The fifth research question is therefore answered affirmatively at the level of formulation: MLLM-based interaction prediction is viable as part of a two-stage grounded system. The strongest adaptations should next be assessed within the complete pipeline using predicted instrument prompts, followed by evaluation of the formulation beyond cholecystectomy.
...
Spatial grounding defines surgical interaction recognition at the level of individual instrument instances. It makes it possible to distinguish multiple actors, associate semantic labels with visible evidence, and evaluate whether an interaction has been assigned to the correct instrument. The resulting task provides a more precise and interpretable description than an unordered set of frame-level interaction labels.
...
Benchmark datasets shape the scientific questions that can be evaluated. Cholec80 enabled systematic study of surgical phase recognition and instrument presence, while CholecT50 supported the development and comparison of surgical action triplet recognition methods~\cite{EndoNet,NWOYE2022102433_rendevous}. In the same way, instrument instance masks are required to assess whether a model distinguishes two graspers in the same frame, and masks aligned with verbs and targets are required to assess whether it assigns the correct interaction to the correct grasper.
...
AP$_{IVT}$ therefore measures a substantially different problem from accuracy in a single, balanced classification task. Component, frequency-stratified, and confusion analyses are needed to interpret the aggregate score and identify the underlying failure modes.
...
Similar distinctions occur between aspiration and irrigation, grasp and retract, or cystic duct and cystic pedicle. A single-component near miss differs from a prediction in which localisation and every semantic component are incorrect. An ontology-valid but visually ambiguous alternative also differs from a malformed or clinically incompatible output. Exact AP remains essential for measuring fully correct grounded predictions, but it does not distinguish semantically close errors from unrelated ones. This motivates the ranked measures and semantically informed evaluation considered next.

\subsection{The Case for \texorpdfstring{Top-$k$}{Top-k} and Richer Evaluation}
\label{sec:discussion-topk}

Future evaluation should retain strict AP while supplementing it with ranked measures and semantically informed costs. Top-$k$ triplet or component evaluation would test whether the reference interaction remains among a model's highest-ranked candidates. In the preceding example, a model that ranks cystic duct second behind cystic artery has narrowed the prediction to a plausible anatomical neighbourhood, whereas a model whose leading candidates are unrelated has failed more fundamentally. Both may have the same top-1 outcome, but they motivate different modelling responses.

Top-$k$ evaluation should not replace the established benchmark or be treated merely as an easier score. Its diagnostic value could be complemented by component-aware results, frequency-stratified reporting, and ontology-aware costs or groupings of related actions and targets. Previous computer-vision work has similarly used class hierarchies to distinguish the severity of different classification mistakes and to inform learning objectives~\cite{bertinetto2020making}. For surgical triplets, any semantic or anatomical cost would require careful definition and validation; it should supplement AP$_{IVT}$ rather than weaken the criterion for exact grounded correctness.
...
Chapter~\ref{chap:two-stage-interaction} establishes MLLM viability but represents an active continuation of the research. Most adaptations are evaluated under ground-truth instrument prompting, so the strongest sequential and temporal formulations require broader evaluation within the complete system using predicted instrument prompts. This limits comparisons among the complete systems.
