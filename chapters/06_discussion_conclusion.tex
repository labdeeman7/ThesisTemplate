...
\section{Development of Spatially Grounded Surgical Interaction Understanding}
...
The fifth research question is therefore answered affirmatively at the level of formulation: MLLM-based interaction prediction works as part of a two-stage grounded system. Continued work is required to assess the strongest adaptations under predicted grounding and determine how well the formulation transfers beyond cholecystectomy. These next steps extend, rather than invalidate, the evidence established in the chapter.
...
The distribution of targets and complete triplets compounds these visual difficulties. A small number of interactions occur frequently, while many valid combinations are rare. Models consequently receive limited supervision for some of the distinctions on which complete-triplet correctness depends. TargetFusionNet shows that learned anatomical fusion can help, but neither it nor the prompt-level prior resolves the underlying ambiguity. Better contact localisation, richer target supervision, and representations that integrate spatial and temporal evidence remain important directions.
...
Chapter~\ref{chap:two-stage-interaction} establishes MLLM viability but represents an active continuation of the research. The principal experiments use one run, and most adaptations are evaluated under ground-truth grounding. The strongest sequential and temporal formulations therefore require broader evaluation within the complete predicted-grounding system. These qualifications limit comparative claims but do not alter the demonstrated feasibility of the two-stage formulation.
...
The most immediate future direction is to test whether the grounded interaction formulation transfers beyond laparoscopic cholecystectomy. ProstaTD is an external, multi-source dataset for spatially localised surgical triplet detection in robot-assisted prostatectomy~\cite{chen2025prostatd}. It provides an opportunity to examine the procedure dependence of the datasets, ontology, grounding strategy, and interaction-prediction models developed in this thesis.
...
The sequential and three-frame formulations in Chapter~\ref{chap:two-stage-interaction} provide the most immediate candidates for further study. They should be evaluated across repeated runs and within the complete predicted-grounding pipeline, both separately and in controlled combinations. Sequential prediction could be made more robust to an incorrect first-turn verb, while curriculum-style adaptation could initialise specialised variants from a converged Direct SFT model rather than independently from the base MLLM.
