...
All configurations use the same official training, validation, and test partitions. In the complete predicted-grounding evaluation, frames for which Stage~1 produces no retained instrument instances provide no Stage~2 input because there is no detected instance for which an interaction can be predicted. Checkpoint selection is based exclusively on the official validation split; the test split is not used for early stopping or model selection.
