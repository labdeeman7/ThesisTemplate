...
cholecystectomy analysis. Generate concise, target-centric reasoning for
provided surgical action triplets. Focus on local instrument-target
interaction and anatomical discrimination.
...
The phrase ``target-centric reasoning'' above is retained because it formed part of the operational teacher instruction. The resulting schema is not limited to target selection: it also records visible contact evidence, action evidence, an alternative target, uncertainty, and the final complete triplet. The chapter therefore uses the broader term \emph{Distilled Reasoning Supervision} for the experiment and does not present target-centric reasoning as an independently evaluated contribution.
