...
The prompts use the six instrument labels, ten verb strings, and fifteen target strings inherited from CholecTriplet-Seg:
...
verb, target. Use only the allowed labels. If idle: verb="no action",
...
For EACH instance ID, assign exactly one (instrument, verb, target)
triplet using visual evidence.

Allowed labels:
{shared instrument, verb, and target vocabularies}
...
Required output format (example):
{
  "predictions": [
    ...
    {"id": "hook_2", "instrument": "Monopolar Hook",
     "verb": "dissect", "target": "cystic plate"}
  ]
}
...
Sequential Dialogue Prediction uses the same grounded image throughout a three-stage dialogue. An earlier instrument-identification turn was not used in the reported experiment. The first retained turn requests the verb for each instance:
...
User: For each labelled instrument instance in this image, identify the
      verb describing the action being performed. Return JSON only.

Assistant response:
{"predictions": [{"id": "grasper_1", "verb": "retract"}]}
\end{lstlisting}
...
User: For each labelled instrument instance, predict the final
      instrument-verb-target triplet. Return JSON only.
...
During training, the complete three-stage dialogue is supplied with the reference model response at each stage, and the token-level cross-entropy in Equation~\ref{eq:c5:sft-loss} is applied to all three responses. During inference, the questions are asked in sequence and the generated verb and target responses remain in the conversation before the final request. Only the final triplet response is parsed for evaluation.

\subsubsection{Ontology-Guided Prompting}

Ontology-Guided Prompting retains the Direct SFT user instruction and response schema but extends the system instruction with the valid verb--target combinations associated with each instrument class. The complete ontology supplied to the model is:
...
Each predicted (instrument, verb, target) MUST exactly match one of the
...
- verb support: 1 sentence, <= 15 words
...
The laparoscopic-field boundary is estimated to identify the likely instrument-entry side and thereby distinguish the proximal base from the distal working end of each instrument. A lightweight deterministic estimate was used because this boundary supports only the auxiliary tip heuristic and does not require an additional trained model. For an RGB image, pixels whose maximum channel intensity exceeds 12 are treated as belonging to the visible laparoscopic field. If these pixels occupy less than 10\% of the image, the field estimate is rejected. Otherwise, the field centre is defined by the median horizontal and vertical coordinates of the valid pixels. The field radius is the 98th percentile of their Euclidean distances from this centre. Estimates containing fewer than 100 valid pixels or having radius below 20 pixels are rejected.

This threshold-based construction is not intended as a general content-area or instrument-pose estimator. More established alternatives could replace it, including robust endoscopic content-area estimation~\cite{budd2023contentarea}, autonomous instrument tracking and tooltip localisation~\cite{gruijthuijsen2022robotic}, and skeletal instrument-pose annotations such as ROBUST-MIPS~\cite{han2025robust}. These approaches were not used in the reported experiment but provide clearer routes for improving the localisation of the context region.
...
\midrule
Sequential Dialogue Prediction & Three supervised assistant responses predict the verb, target, and final triplet in sequence; preceding reference responses are supplied through teacher forcing during training. \\
...
Ontology-Guided Prompting & Valid verb--target combinations are appended to the prompt during training and evaluation; decoding remains unconstrained. \\
...
All configurations use the same official training, validation, and test partitions. In the complete evaluation with predicted instrument prompting, frames for which Stage~1 produces no retained instrument instances provide no Stage~2 input because there is no detected instance for which an interaction can be predicted. Checkpoint selection is based exclusively on the official validation split; the test split is not used for early stopping or model selection.
...
\begin{figure}[p]
    ...
    \caption{Row-normalised confusion matrices for Direct SFT under ground-truth instrument prompting. The upper matrix reports actions and the lower matrix reports targets. Rows denote ground-truth classes and columns denote predicted classes.}
...
\begin{figure}[p]
    ...
    \caption{Row-normalised confusion matrices for three-frame temporal context under ground-truth instrument prompting. The upper matrix reports actions and the lower matrix reports targets. Rows denote ground-truth classes and columns denote predicted classes.}
...
\begin{figure}[p]
    ...
    \caption{Row-normalised confusion matrices for the multitask model evaluated with the Direct SFT response instruction under ground-truth instrument prompting. The upper matrix reports actions and the lower matrix reports targets. Rows denote ground-truth classes and columns denote predicted classes.}
