...
- Layout integration: the main figures and tables retain flexible single-column placement near their discussion; the six confusion matrices remain on separate float pages for legibility; and the supplementary appendix has its own running-header mark. Local emergency line-breaking tolerance is applied by the wrapper. The provisional qualitative error-analysis panel was removed from the thesis because no systematically selected qualitative comparison set was available; the underlying source material remains preserved outside the active chapter.
