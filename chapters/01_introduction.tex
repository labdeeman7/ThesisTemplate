...
Substantial progress has been made on individual surgical vision tasks. Models can recognise instruments, segment their visible regions, identify operative phases, and classify surgical activities. Nevertheless, surgical scene understanding is not simply a collection of independent labels. A meaningful description of a scene is relational: it should identify which instrument is present, where that particular instance is located, what action it is performing, and which anatomical structure it is acting upon. This thesis addresses that relational level of understanding by progressing from instrument perception to spatially grounded instrument--tissue interaction prediction.
...
This thesis connects these previously separate lines of research. It first establishes the spatial foundation through instrument instance segmentation, then links instrument masks to action triplets, and finally investigates models that predict interactions for those grounded instances. The resulting progression is from recognising what is present in a frame to identifying which instrument instance is responsible for each predicted interaction.
...
The review identifies a progression from coarse workflow recognition towards increasingly fine-grained activity descriptions. Surgical action triplets represent an important step in that progression, but their predominantly frame-level formulation leaves the acting instance ambiguous. At the same time, instance segmentation had received substantially less attention than instrument presence recognition, detection, or semantic segmentation. The field therefore lacked the combined supervision required to connect a semantically rich triplet to the precise instrument responsible for it.
...
\begin{enumerate}
  ...
  \item Can surgical action triplets be grounded to instrument instance masks, enabling spatially interpretable \ivttriplet predictions?
...
These questions form a cumulative programme of work. The review establishes the conceptual and methodological gap. CholecInstanceSeg provides the necessary instance-level spatial supervision. CholecTriplet-Seg connects that supervision to surgical action triplets. TargetFusionNet investigates target-aware grounded prediction within a unified architecture. The final technical chapter then examines whether localisation and semantic interaction prediction can be divided between specialised models.
