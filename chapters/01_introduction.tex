...
Datasets released by the CAMMA research group have been central to surgical video analysis in laparoscopic cholecystectomy. Cholec80 supported research on surgical phase recognition and instrument-presence detection \cite{EndoNet}. Building on this foundation, the CholecT series, particularly CholecT50, supported the development of surgical action triplet recognition \cite{NWOYE2022102433_rendevous}. TripNet and Rendezvous established influential modelling approaches for learning relationships between instruments, verbs, and targets \cite{1nywoye_rec_action_triplets,NWOYE2022102433_rendevous}. Their associated benchmarks enabled systematic comparison on fine-grained activity recognition. However, progress in recognising triplet labels did not by itself resolve where each interaction occurred.
...
The complete pipeline is evaluated using predicted instrument instances, rather than only ground-truth instrument prompting. The results establish the viability of instrument instance segmentation followed by multimodal interaction prediction and expose the effect of first-stage errors on the complete system. This chapter represents a forward-looking continuation of the dataset and grounded-modelling contributions, rather than replacing their central role in the thesis.

\section{Thesis Scope}
\label{sec:introduction-scope}

From a clinical application perspective, the thesis focuses on laparoscopic cholecystectomy, for which established public datasets make it possible to develop a coherent sequence of instance-segmentation, triplet-grounding, and interaction-prediction studies. The shared procedural setting allows the contributions to build upon Cholec80, CholecT50, and CholecSeg8k while maintaining consistent definitions of instruments, actions, and anatomical targets.
